...
var globalSkuID='20057';

$('#ddlskudropdown').change(function ()
{       
    debugger;
    globalSkuID = $("#ddlskudropdown").val();
});
...
function openReportPopup(SkuId) {
     globalSkuID=SkuId
...
$(document).ready(function () 
{
    ...
    SkuDropdownstockledger();


})

function SkuDropdownstockledger()
{     
    //debugger;
    var apiPath = wmsApiPath + 'ReportStatic/getStockledgerSkuDropdown'; 
    var getUserId = mBrillWmsSession.getUserId();
    ...
    //var apiPath = 'http://localhost:50068/api/staging/v1/ReportStatic/getStockledgerSkuDropdown';
    ...
    callHttpUrl(apiPath,postData, function (data)
    {
        ...
        if (isStatus == 200) {
            spnSkutype = [];
            spnSkutypeId = [];
            $("#ddlskudropdown").html('<option >--Select--</option>');
            spnSkutype.push('--Select List--');
			spnSkutypeId.push('0');
            var myGridList = data.Result;
            ...
            if (grnContainerSize.length > 0) {
                for (var i = 0; i < grnContainerSize.length; i++) 
                    {
                    var SkuId =grnContainerSize[i].Id;
                    var SkuProductName = grnContainerSize[i].ProductName;
                    spnSkutype.push(SkuProductName);
                    spnSkutypeId.push(SkuId);
                    $("#ddlskudropdown").append('<option value = "'+ SkuId +'">'+ SkuProductName +'</option>');
                    }
            }
        }        
    });    
...
function getPurchaseOrderList(getSelectedDateCatg, fromDate, toDate) 
{
    ...
    var getSearchToDate = "";
    var apiPath = wmsApiPath + "ReportStatic/getStockledgerReport";
    ...
    callHttpUrl(apiPath, postData, function (data) {
        ...
        if (getStatus == 200) {
            var getResult = data.Result.Table1;
            var getResult1 = data.Result.Table;
            if (obj == 'objrexta') 
            {
                ...
                // myGrid = myGrid + '<div class="wms-srv-grid-cell reportHeaderCell" style="text-align:center; color: #b3b3b3;font-weight: 500;width:198px">SKU Code</div>';
                // myGrid = myGrid + '<div class="wms-srv-grid-cell reportHeaderCell" style="text-align:center; color: #b3b3b3;font-weight: 500;width:213px">Description</div>';
                // myGrid = myGrid + '</div>';

                myGrid = myGrid + '<div class="wms-srv-grid-header reportHeaderRow" id="header-wrap">';
                myGrid = myGrid + '<div class="wms-srv-grid-cell reportHeaderCell" style="text-align:center; color: #b3b3b3;font-weight: 500;">Warehouse</div>';
                myGrid = myGrid + '<div class="wms-srv-grid-cell reportHeaderCell" style="text-align:center; color: #b3b3b3;font-weight: 500;">Pallet Name</div>';
                ...
                myGrid = myGrid + '<div class="wms-srv-grid-cell reportHeaderCell" style="text-align:center;color: #b3b3b3;font-weight: 500;">Opening Balance</div>';
                myGrid = myGrid + '<div class="wms-srv-grid-cell reportHeaderCell" style=" text-align:center;color: #b3b3b3;font-weight: 500;">In Qty</div>';
                myGrid = myGrid + '<div class="wms-srv-grid-cell reportHeaderCell" style="text-align:center;color: #b3b3b3;font-weight: 500;">Out Qty</div>';
                myGrid = myGrid + '<div class="wms-srv-grid-cell reportHeaderCell" style="text-align:center;color: #b3b3b3;font-weight: 500;">Cycle Count Qty</div>';
                myGrid = myGrid + '<div class="wms-srv-grid-cell reportHeaderCell" style="text-align:center;color: #b3b3b3;font-weight: 500;">Transfer Qty</div>';
                ...
                {
                    var getwarehouse = getResult[i].warehouse;
                    var getPalletName = getResult[i].PalletName;
                    var getLocCode = getResult[i].LocCode;
                    var getopstk = getResult[i].opstk;
                    var getInQty = getResult[i].InQty;
                    var getOutQty = getResult[i].OutQty;
                    var getcyclecount = getResult[i].cyclecount;
                    var getTransferQty = getResult[i].TransferQty;
                    var getClosingBalance = getResult[i].ClosingBalance;
                    ...
                    // myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;">' + getSKUId + '</div>';
                    // myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;">' + getDescription + '</div>';
                    // myGrid = myGrid + '</div>';
                    $("#txtsku").html(getSKUId);
                    $("#txtdes").html(getDescription);
    
                    myGrid = myGrid + '<div id="" class="wms-srv-grid-row reportRow">';
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;">' + getwarehouse + '</div>';
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;">' + getPalletName + '</div>';
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;">' + getLocCode + '</div>';
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;">' + getopstk + '</div>';
                    ...
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;">' + getcyclecount + '</div>';
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;">' + getTransferQty + '</div>';
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;">' + getClosingBalance + '</div>';
                    myGrid = myGrid + '</div>';
             ...
             for(var j=0; j< getResult1.length;j++)
             {
                var getTotalInQty = getResult1[j].TotalInQty;
                var getTotalOutQty = getResult1[j].TotalOutQty;
                var getCycleCountQty = getResult1[j].CycleCountQty;
                var getTransferQty = getResult1[j].TransferQty;
                var getClosingBalance = getResult1[j].ClosingBalance;
             }
             $("#txtInqty").html(getTotalInQty);
             $("#txtOutQty").html(getTotalOutQty);
             $("#txtCycleCountQty").html(getCycleCountQty);
             $("#txtTransferQty").html(getTransferQty);
             $("#txtClosingBalance").html(getClosingBalance);
            ...
            if (obj == 'objrexta') 
            {
                ...
                for (var i = 0; i < getResult.length; i++) 
                {
                    var getTotalInQty = getResult[i].TotalInQty;
                    var getTotalOutQty = getResult[i].TotalOutQty;
                    var getCycleCountQty = getResult[i].CycleCountQty;
                    var getTransferQty = getResult[i].TransferQty;
                    var getTransferQty = getResult[i].TransferQty;
    
                    myGrid = myGrid + '<div id="" class="wms-srv-grid-row reportRow">';
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;width:79px;">' + getTotalInQty + '</div>';
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;width:97px;">' + getTotalOutQty + '</div>';
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;width:184px;">' + getCycleCountQty + '</div>';
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;width:146px;">' + getTransferQty + '</div>';
                    myGrid = myGrid + '<div class="wms-srv-grid-cell reportRowCell" style="text-align:center;width:184px;">' + getTransferQty + '</div>';
                    myGrid = myGrid + '</div>';
                }
                $('#gettopfiveorder' + obj).html(myGrid);
            }
