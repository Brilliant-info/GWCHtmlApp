...
//var wmsApiPathimp = 'http://38.242.245.55/EasyWMSImportAPI/api/staging/v1/';
var importObject = "";
var templateId = '0';
...
function getimportobject() {
    ...
    callHttpUrl(apiPath, postData, function (data) {
        ...
        if (getStatus == "success") {
            ...
            // $('#import_wizard_grid').html('');
            var gridTable = '';
            //  gridTable = gridTable + '<div class="row import_wizard">';
            //  gridTable = gridTable + '<div class="col-lg-6 import_wizard_left"></div>';
            ...
            for (var i = 0; i < Objectlist.length; i++) {
                ...
                // gridTable = gridTable + '<div class="importMenuItems btnopenimportwizardsteps">';
                // gridTable = gridTable + '<div class="fas fa-list">' + ObjectName + '</div>';
                //  gridTable = gridTable + '</div>';
                $('.btnopenimportwizardsteps[data-obj="' + ObjectName + '"]').show();
            }

            // $('#import_wizard_grid').html(gridTable);
...
function downloadTemplate(tmpltype) {
    ...
    debugger;
    var apiPath = wmsApiPathimp + 'generateimpTemplate';
    var postData = {
    ...
    if (templateId != '0') {
        callHttpUrl(apiPath, postData, function (data) {
            ...
            if (getStatus == "success") {
                ...
                // window.location.href = getImportfilepath;
                // document.getElementById("exceltemp").href=getImportfilepath;

                // var pathexc =  "http://localhost/download/" + getImportfilepath;
                //var pathexc =  wmsAppFiles +"Document/"+ getImportfilepath;
               // var pathexc = "http://173.212.244.46/BWMSImportAPI2.1/ImportDesign/ImportTemplate/" + getImportfilepath;  
                var pathexc = "http://38.242.245.55/EasyWMSImportAPI/ImportDesign/ImportTemplate/" + getImportfilepath;
                http://38.242.245.55/EasyWMSImportAPI/

                //  var pathexc =  "http://164.68.123.40/download/" + getImportfilepath;
                // window.open(pathexc);
                window.location.href = pathexc;
    ...
    //         //var objectName = hdnImportObject.value
    //			importObject
    //         window.location.href = "DownloadTemplate.ashx?tid=" + templateId + "&type=" + tmpltype + "&ObjectName=" + importObject +"&Customerid=" + CustomerId +"&CompanyID=" + CompanyID;
    //     }
    //     else {
    //         alert('Error to load template details for selected customer!!');
    //     }
