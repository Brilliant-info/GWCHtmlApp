...
function loadInboundList() {
    ...
    var apiPath = wmsApiPath + 'PO/GetInboundList';
    //var apiPath = "http://localhost:50068/api/staging/v1/PO/GetInboundList"

    var getGroupName = "chkinwargroup";

    var getActiveTab = $('.dashboardActiveTab').attr('data-id');
    if (getActiveTab == 'QualityCheck') {
        $(".ddlMorebtn").hide();
    }
    if (getActiveTab == 'Put Away') {
        $(".ddlMorebtn").hide();
    }
    if (getActiveTab == 'Receiving') {
        $(".ddlMorebtn").show();
    }
    if (getActiveTab == 'Inward Orders') {
        $(".ddlMorebtn").show();
    }
    ...
    callHttpUrl(apiPath, postData, function (data) {
        ...
        if (getStatusCode == 'Success') {
            ...
            //var getCurrentPage = myGridList[0].CurrentPage;
            ...
            var getTaskassign = myGridListcount[0].Taskassign;
            ...
            $('#dashCompletelOrder').html(getCompleted);
            $('#dashReceiving').html(getReceiving);
            $('#dashPutAway').html(getPutAway);
            $('#dashExpectedReceipt').html(getExpectedReceipt);
            $('#dashQualityCheck').html(getQualitycheck);
            ...
            if (hastaskassign == false) {
                $('.wms-srv-count-box-holder[data-id="CompletedOrder"]').parent().hide();
                $('.wms-srv-count-box-holder').each(function () {
                    var getMyParent = $(this).parent();
                    var hasClass = $(getMyParent).hasClass('col-md-3');
                    if (hasClass) {
                ...
                $('.wms-srv-count-box-holder[data-id="CompletedOrder"]').parent().show();
                $('.wms-srv-count-box-holder').each(function () {
                    var getMyParent = $(this).parent();
                    var hasClass = $(getMyParent).hasClass('col-md-4');
                    if (hasClass) {
            ...
            if (myGridList != "") {
                ...
                gridTable = gridTable + '<div class="wms-srv-grid-cell">Vendor Name</div>';
                gridTable = gridTable + '<div class="wms-srv-grid-cell">Order Date</div>';
                gridTable = gridTable + '<div class="wms-srv-grid-cell">Total Quantity</div>';
                ...
                for (var i = 0; i < myGridList.length; i++) {
                    var getExpectedReceiptNo = myGridList[i].ExpectedReceiptNo;
                    var getReferenceNo = myGridList[i].ReferenceNo;
                    var getWarehouseCode = myGridList[i].WarehouseCode;
                    var getOrderDate = myGridList[i].OrderDate;
                    var getContainerId = myGridList[i].ContainerId;
                    var getOrdStatus = myGridList[i].Status;
                    var getOrdType = myGridList[i].OrderType;
                    var getvendorName = myGridList[i].vendorName;
                    var getTotalQty = myGridList[i].TotalQty;
                    ...
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getOrdType + '</div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getReferenceNo + '</div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getvendorName + '</div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getOrderDate + '</div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getTotalQty + '</div>';
                    // gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getContainerId +'</div>';
                    var objectHeadData = "Order No|" + getExpectedReceiptNo + ",Order Date|" + getOrderDate + ",Order Reference No|" + getReferenceNo + ",Warehouse Code|" + getWarehouseCode + "";
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + bindStatusControl(getExpectedReceiptNo, getOrdStatus) + '</div>';
                ...
                setupGridPagingList('tlbInboundGridPager', CurrentPage, getTotalRecords, loadInboundList, getglobalTotalRecords);
            }
            // else{
            //     $('#dashCancelOrder').html('0');
