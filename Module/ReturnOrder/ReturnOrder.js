...
//function loadNormalReturnOrderList() {
//    var getOrderType = $('#ddlRequestOrderType').val();
//    if (getOrderType == 'NormalOrder') {
//        loadNormalReturnOrderList();
//    } else {
//        loadEcommerceReturnOrderList();
//    }
//}

function loadEcommerceReturnOrderList() {
    /* Static API Call */
    apiPath = "StaticAPI/GetEcommerceReturnOrderList.json";
    postData = null;
    /* Static API Call */
            ...
            if (myGridList != "") {
                ...
                for (var i = 0; i < myGridList.length; i++) {
                    ...
                    gridTable = gridTable + '<div class="wms-srv-grid-cell"><div class="wms-srv-status-holder" title="Status">' +
                        ...
                        '<div class="wms-srv-status-label">Request</div>' +
                ...
                setupGridPagingList('tlbInboundGridPager', CurrentPage, getTotalRecords, loadNormalReturnOrderList, getglobalTotalRecords);
            }
        }
        else {
            alert('Unable to connect Server!!');
...
function loadNormalReturnOrderList() {
    /* Static API Call */
    apiPath = "StaticAPI/GetReturnOrderList.json";
    postData = null;
    /* Static API Call */
    ...
    callHttpUrl(apiPath, postData, function (data) {
        ...
        if (getStatusCode == 'Success') {
            ...
            if (myGridList != "") {
                ...
                gridTable = gridTable + '<div class="wms-srv-grid-header" style="text-align: center;">';
                gridTable = gridTable + '<div class="wms-srv-grid-cell"></div>';
                gridTable = gridTable + '<div class="wms-srv-grid-cell">Return No</div>';
                gridTable = gridTable + '<div class="wms-srv-grid-cell">Department</div>';
                gridTable = gridTable + '<div class="wms-srv-grid-cell">Return Date</div>';
                gridTable = gridTable + '<div class="wms-srv-grid-cell">OMS Ref. Number</div>';
                gridTable = gridTable + '<div class="wms-srv-grid-cell">Priority</div>';
                gridTable = gridTable + '<div class="wms-srv-grid-cell">Return Category</div>';
                gridTable = gridTable + '<div class="wms-srv-grid-cell">Location Name</div>';
                gridTable = gridTable + '<div class="wms-srv-grid-cell" style="width:260px;">Status</div>';
                gridTable = gridTable + '<div class="wms-srv-grid-cell">Action</div>';
                gridTable = gridTable + '</div>';
                // GRID HEADER
                ...
                for (var i = 0; i < myGridList.length; i++) {
                    var getID = myGridList[i].ReturnID;
                    var getReturnNo = myGridList[i].ReturnID;
                    var getDepartment = myGridList[i].Department;
                    var getReturnDate = myGridList[i].returndate;
                    var getOMSRefNo = myGridList[i].referenceNo;
                    var getPriority = myGridList[i].Priority;
                    var getReturnCategory = myGridList[i].returncategoryName;
                    var getLocationName = myGridList[i].LocationName;
                    var getStatus = myGridList[i].Status;

                    // GRID ROW
                    gridTable = gridTable + '<div class="wms-srv-grid-row" style="text-align: center;">';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell"><input type="checkbox" onclick="selectAllTask(This);" class="messageCheckbox" data-id="' + getReturnNo + '" ></div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getReturnNo + '</div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getDepartment + '</div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getReturnDate + '</div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getOMSRefNo + '</div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getPriority + '</div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getReturnCategory + '</div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell">' + getLocationName + '</div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell"><div class="wms-srv-status-holder" title="Status">' + 
                       '<div class="wms-srv-status-display">' + 
                            '<div class="wms-srv-status-label">Pending Driver Alloc.</div>' +
                            '<div class="wms-srv-status-progress">' +
                        '<div class="wms-srv-dot wms-srv-red" data-orderid="' + getReturnNo + '" data-status="Driver Allocation" title="Driver Allocation" onclick="viewOrder(\'' + getReturnNo + '\', true);return false;"></div>' +
                        ' <div class="wms-srv-dot wms-srv-gray" data-orderid="' + getReturnNo + '" data-status="Collection In Progress" title="Collection In Progress" onclick="openApprovalTrans(\'' + getReturnNo + '\');return false;"></div>' +
                        ' <div class="wms-srv-dot wms-srv-gray" data-orderid="' + getReturnNo + '" data-status="Rec.In Warehouse" title="Rec.In Warehouse" onclick="openDispatchPopup(\'' + getReturnNo + '\');return false;"></div>' +
                            '</div>' +
                            '<div class="wms-srv-status-action"><a href="#" class="wms-srv-btn-status-action"><i class="fas fa-play-circle"></i></a></div>' +
                        '</div>' +
                       '</div ></div>';
                    gridTable = gridTable + '<div class="wms-srv-grid-cell"><div class="wms-srv-grid-action">' + 
                        '<a href = "#" title = "Document" data-prefix="DOC" data-orderid="' + getReturnNo + '" class="wms-srv-icononly" onclick="openDocumentObject(\'' + getReturnNo + '\');"><i class="fas fa-file-alt"></i></a>' +
                        '<div class="wms-srv-action-sep" >|</div> ' +
                        '<a href = "#" title = "Allocate Driver" data-prefix="CMM" data-orderid="' + getReturnNo + '" class="wms-srv-icononly" onclick="showAllocateDriverPopup(\'' + getReturnNo + '\');" > <i class="fas fa-tasks iconView"></i></a> ' +
                        // '<div class="wms-srv-action-sep" >|</div> ' +
                        // '<a href = "#" title = "Approval" data-prefix="APR" data-orderid="' + getRequestNo + '" class="wms-srv-icononly" onclick="openApprovalTrans(\'' + getRequestNo + '\');" > <i class="fas fa-user-check"></i></a> ' +
                        // '<div class="wms-srv-action-sep" >|</div> ' +
                        // '<a href = "#" title = "Dispatch" data-prefix="TRN" data-orderid="' + getRequestNo + '" class="wms-srv-icononly" onclick="openDispatchPopup(\'' + getRequestNo + '\');" > <i class="fas fa-truck-moving fa-flip-horizontal"></i></a> ' +
